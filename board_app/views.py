...
from .filters import AdvertisementFilter, ResponseFilter
from .forms import AdvertisementForm, ResponseForm, UpdateResponseForm
...
class ResponseUpdate(UpdateView):
    ...
    def confirm(self, request, *args, **kwargs):
        Response.objects.filter(id=self.kwargs.get('pk')).update(status=True)
        Response.save()
        return redirect(f'/advertisements/responses/{Response.id}')
...
class CommentatorresponsesList(ListView):
    model = Response
    ordering = '-time_in'
    template_name = 'responses_of_author.html'
    context_object_name = 'commentatorresponses'
    paginate_by = 10

    def get_queryset(self):
        self.commentator = get_object_or_404(User, id=self.kwargs['pk'])
        queryset = Response.objects.filter(commentator=self.commentator)
        return queryset
